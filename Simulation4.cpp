...
void createHistogram(double binSize, int totalParticles, int particlesType1,
		...
		int rightBoundary, double currentTime);

void loadStateFromFile(ifstream& loadStateFile, double* positionX,
	double* positionY, double* oldPositionX, double* oldPositionY,
	double* velocityX, double* velocityY, double& currentTime,
	int totalParticles);

void saveStateToFile(ofstream& loadStateFile, double* positionX,
	double* positionY, double* oldPositionX, double* oldPositionY,
	double* velocityX, double* velocityY, double currentTime,
	int totalParticles);
...
//Create data files
	...
	double timestep = 0.00125; //Can be arbitrarily small
	double maxTime = 0.1; //Can be arbitrarily long or short
	double binSize = 0.1;
	double coolingFactor = 0.75;
...
//THIS MUST BE MODIFIED IF YOU USE MORE PARTICLES
	...
	{
		ifstream infile;
		double reductionFactor = atof(argv[2]);
		infile.open(argv[1]);
		loadStateFromFile(infile, positionX,
		positionY, oldPositionX, oldPositionY,
		velocityX, velocityY, currentTime, totalParticles);
		infile.close();

		//Is kinetic energy actually being reduced?
		for (int i = 0; i < totalParticles; i++)
		{
			velocityX[i] *= reductionFactor;
			velocityY[i] *= reductionFactor;
		}		
	}
	else
	{
		...
		//Perform initial Euler operation to set things in motion if starting with random location
	...
	/*for (int i = 0; i < 3; i++) //Number of times want to cool
	 {
	 cout<<"Now working on round "<<i<<"."<<endl;*/
	double curTime = currentTime;
	//for (currentTime += timestep; currentTime < maxTime + curTime;
	//		currentTime += timestep)

	
	for (currentTime; currentTime < (maxTime + curTime); currentTime += timestep)
	{
		//cout << currentTime<< " " << maxTime + curTime << endl;
		positionFile << "* " << currentTime << endl;
...
//	createHistogram(binSize, totalParticles, numParticlesType1, positionX,
	//		positionY, upperBoundary, rightBoundary, currentTime);
	
	//ofstream saveStateFile;
	//saveStateFile.open("saveState.txt");

	//saveStateToFile(saveStateFile, positionX,
		//positionY, oldPositionX, oldPositionY,
		//velocityX, velocityY, currentTime, totalParticles);
	...
	/*	//Cool particles
	 ...
	 {
	 velocityX[j] *= coolingFactor;
	 velocityY[j] *= coolingFactor;
	 }*/
//	}
	//Deleting arrays					
	delete positionX;
	...
	for (int i = 0; i < totalParticles; i++)
	{
		for (int j = 0; j < i; j++)
		{
			...
			else
			{
				...
				//This can be optimized further by removing power functions
				...
				/*forceBetweenX[i][j] = ((24 * xvector * pow(sigma, 6))
				/ (pow(pythagorean, 4)))
				* ((2 * pow(sigma, 6) / (pow(pythagorean, 3))));
				forceBetweenY[i][j] = ((24 * yvector * pow(sigma, 6))
				/ (pow(pythagorean, 4)))
				* ((2 * pow(sigma, 6) / (pow(pythagorean, 3))));
				potentialEnergy += 4
		...
		int rightBoundary, double currentTime)
{
	int maxSize = (int) (sqrt(
			(double) (rightBoundary * rightBoundary
					+ upperBoundary * upperBoundary)) / (binSize));
...
void loadStateFromFile(ifstream& loadStateFile, double* positionX,
	...
	int totalParticles)
{
	loadStateFile >> currentTime;
	for (int i = 0; i < totalParticles; i++)
	{
		loadStateFile >> positionX[i];
		loadStateFile >> positionY[i];
		loadStateFile >> oldPositionX[i];
		loadStateFile >> oldPositionY[i];
		loadStateFile >> velocityX[i];
		loadStateFile >> velocityY[i];
	}
}

void saveStateToFile(ofstream& saveStateFile, double* positionX,
	double* positionY, double* oldPositionX, double* oldPositionY,
	double* velocityX, double* velocityY, double currentTime,
	int totalParticles)
{
	saveStateFile << currentTime << "\n";
	for (int i = 0; i < totalParticles; i++)
	{
		saveStateFile << positionX[i] << " " << positionY[i] << " "
			<< oldPositionX[i] << " " << oldPositionY[i] << " "
